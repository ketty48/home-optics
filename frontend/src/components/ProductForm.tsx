...
import { Loader, Upload, Trash2, Star, ArrowLeft } from 'lucide-react';
...
const ProductForm = ({
  title, subtitle, submitLabel, SubmitIcon,
  formData, setFormData, loading, onSubmit,
}: ProductFormProps) => {
    ...
    <div className="p-3 sm:p-6 max-w-3xl w-full">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 truncate max-w-xs">{subtitle}</p>
        </div>
      </div>
      ...
      <form onSubmit={onSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-xl shadow-sm p-5 space-y-4">
              ...
              {formData.images.map(image => (
                <div key={image.url} className="relative group">
                  ...
                  <div className="absolute inset-0 bg-black/50 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    <button type="button" onClick={() => handleSetMain(image.url)} className={`${image.isMain ? 'text-yellow-400' : 'text-white hover:text-yellow-400'}`} title="Set as main">
                      <Star size={15} className={image.isMain ? 'fill-current' : ''} />
                    </button>
                    <button type="button" onClick={() => handleRemoveImage(image.url)} className="text-white hover:text-red-400" title="Remove">
                      <Trash2 size={15} />
                    </button>
                  </div>
          ...
          <p className="text-xs text-gray-400">Click the star to set the main image. First uploaded image is main by default.</p>
        </div>
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <button type="submit" disabled={loading} className="flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-semibold text-white disabled:opacity-60 transition-opacity" style={{ backgroundColor: '#1a56db' }}>
            {loading ? <Loader size={15} className="animate-spin" /> : <SubmitIcon size={15} />}
            {submitLabel}
          </button>
        </div>
      </form>
