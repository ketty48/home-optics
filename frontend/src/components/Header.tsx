...
const Header = () => {
  ...
  return (
    <header style={{ fontFamily: "'Nunito', 'Segoe UI', sans-serif" }} className="sticky top-0 z-50 bg-white shadow-sm">
      {/* Main Header */}
      <div>
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-4">
          ...
          <Link to="/" className="flex items-center gap-2 flex-shrink-0" style={{ textDecoration: 'none' }}>
            ...
            <div className="hidden md:block">
              <span style={{ color: '#0a1628', fontWeight: 900, fontSize: 20 }}>Home</span>
              <span style={{ color: '#1a56db', fontWeight: 900, fontSize: 20 }}>Optics</span>
            </div>
          </Link>

          {/* Search */}
          <div className="flex-1 max-w-3xl">
            <form onSubmit={handleSearch} style={{ display: 'flex', backgroundColor: '#f0f4ff', borderRadius: 6, overflow: 'hidden', border: '1.5px solid #e0e7ff' }}>
              <select
                value={searchCategory}
                onChange={e => setSearchCategory(e.target.value)}
                className="hidden md:block"
                style={{ border: 'none', borderRight: '1.5px solid #e0e7ff', padding: '0 12px', fontSize: 13, color: '#333', backgroundColor: 'transparent', outline: 'none', minWidth: 150 }}
              >
              ...
              <input
                ...
                style={{ flex: 1, padding: '10px 16px', border: 'none', outline: 'none', fontSize: 14, backgroundColor: 'transparent' }}
              />
          ...
          <div className="flex items-center gap-4 flex-shrink-0">
            <Link to="/cart" style={{ color: '#475569', position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', textDecoration: 'none' }} className="hover:text-blue-600">
              ...
              <span style={{ fontSize: 10, marginTop: 2, fontWeight: 600 }}>Cart</span>
            </Link>

            {isAuthenticated ? (
              <div className="group" style={{ position: 'relative' }}>
                <button style={{ color: '#475569', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'none', border: 'none', cursor: 'pointer' }} className="hover:text-blue-600">
                  <User style={{ width: 24, height: 24 }} />
                  <span style={{ fontSize: 10, marginTop: 2, fontWeight: 600 }}>{user?.firstName}</span>
                </button>
              ...
              <Link to="/login" style={{ color: '#475569', display: 'flex', flexDirection: 'column', alignItems: 'center', textDecoration: 'none' }} className="hover:text-blue-600">
                <User style={{ width: 24, height: 24 }} />
                <span style={{ fontSize: 10, marginTop: 2, fontWeight: 600 }}>Sign In</span>
              </Link>
            )}

            <button className="md:hidden" style={{ color: '#475569', background: 'none', border: 'none', cursor: 'pointer' }} onClick={() => setIsMenuOpen(!isMenuOpen)}>
          ...
          <div className="px-4 py-3">
            ...
            <nav className="flex flex-col">
              {[
                ...
                <Link key={link.label} to={link.to} style={{ color: '#333', padding: '10px 0', fontSize: 14, textDecoration: 'none', borderBottom: '1px solid #f0f4ff' }} onClick={() => setIsMenuOpen(false)}>
                  {link.label}
                </Link>
              ))}
            </nav>
